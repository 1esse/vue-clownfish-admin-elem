...
const getNavIcon = (item: RouteMeta | undefined) => {
  ...
  if (typeof item.icon === 'string') {
    return <ElIcon><SvgIcon iconName={item.icon as string} /></ElIcon>
  }
  return <ElIcon>{h(item?.icon as Component)}</ElIcon>
}
...
const TheSideBar = () => (
  <Scrollbar>
    <ElMenu class="menu-vertical" mode="vertical" collapse={sidebarRelated?.collapsed} defaultActive={defaultActive.value} defaultOpeneds={defaultOpeneds.value} collapseTransition={true}>
