  ...
  const menuItemTemplate = (route: RouteRecordRaw) => {
    ...
    return (
      <MenuItemLink route={route} url={url}>
        <ElMenuItem index={url} v-slots={slots} route={route}>
          {getNavIcon(route.meta)}
        </ElMenuItem>
      </MenuItemLink>
    )
...
const TheSideBar = () => (
  <Scrollbar>
    <ElMenu class="menu-vertical" router={true} mode="vertical" collapse={sidebarRelated?.collapsed} defaultActive={defaultActive.value} defaultOpeneds={defaultOpeneds.value} collapseTransition={false}>
